...
class TenderContractResourceTest(BaseTenderWebTest):
    ...
    def test_patch_tender_contract(self):
        ...
        self.assertEqual(response.json['data']['items'][0], contract['items'][0])
        ...
        response = self.app.get('/tenders/{}/contracts/{}?acc_token={}'.format(self.tender_id, contract['id'], self.tender_token))
        self.assertEqual(response.status, '200 OK')
        self.assertEqual(response.content_type, 'application/json')
        self.assertEqual(response.json['data']["status"], "active")
        self.assertEqual(response.json['data']["value"]['amount'], 238)
...
class TenderContractChangeStatusTest(BaseTenderWebTest):
    initial_status = 'active.qualification'
    initial_bids = test_bids
